...
@triton.jit
def _quant_bmm_chunk_fwd_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        a = tl.load(a_scale).to(dot_dtype) * tl.load(a_ptrs, mask=(offs_m[:, None] < chunk_size_limit) & (offs_k[None, :] < K - k * BLOCK_SIZE_K), other=0.0).to(dot_dtype)
        b = tl.load(b_scale).to(dot_dtype) * tl.load(b_ptrs, mask=(offs_k[:, None] < K - k * BLOCK_SIZE_K) & (offs_n[None, :] < chunk_size_limit), other=0.0).to(dot_dtype)
        acc += tl.dot(a, b)
...
def _quant_bmm_chunk_fwd(a, a_scale, b, b_scale, chunk_size, seq_idx=None, causal=False, output_dtype=None):
    """
    Argument:
    ...
    with torch.cuda.device(a.device.index):
        _quant_bmm_chunk_fwd_kernel[grid](
