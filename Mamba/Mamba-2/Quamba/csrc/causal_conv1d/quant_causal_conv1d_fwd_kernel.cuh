...
template<typename Ktraits>
__global__ __launch_bounds__(Ktraits::kNThreads)
void quant_causal_conv1d_channellast_fwd_kernel(QuantConvParamsBase params) {
    constexpr int kWidth = Ktraits::kWidth;
    constexpr int kNThreads = Ktraits::kNThreads;
    constexpr int kNElts = Ktraits::kNElts;
    ...
    for (int i = 0; i < kLPerThread; ++i) {
        out_vals[i] = scale_b * bias_val;
        #pragma unroll
        for (int w = 0; w < kWidth; ++w) {
            out_vals[i] += scale_wx * weight_vals[w] * x_vals[i + w];
        }
    ...
    for (int i = 0; i < kLPerThread; ++i) {
        int tmp = int(roundf(out_vals[i] / scale_out));
        x_smem[col_idx * kLPerThread + i][row_idx] =  tmp > 127 ? 127 : tmp < -128 ? -128 : static_cast<input_t>(tmp);
    }
    __syncthreads();
...
template<int kNThreads, int kWidth, typename input_t, typename weight_t>
void quant_causal_conv1d_channellast_fwd_launch(QuantConvParamsBase &params, cudaStream_t stream) {
    ...
    auto kernel = &quant_causal_conv1d_channellast_fwd_kernel<Ktraits>;
    // if (kSmemSize >= 48 * 1024) {
    //     C10_CUDA_CHECK(cudaFuncSetAttribute(
    //         kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, kSmemSize));
    //     }
...
void quant_causal_conv1d_channellast_fwd_cuda(QuantConvParamsBase &params, cudaStream_t stream) {
    if (params.width == 2) {
        quant_causal_conv1d_channellast_fwd_launch<128, 2, input_t, weight_t>(params, stream);
    } else if (params.width == 3) {
        quant_causal_conv1d_channellast_fwd_launch<128, 3, input_t, weight_t>(params, stream);
    } else if (params.width == 4) {
        quant_causal_conv1d_channellast_fwd_launch<128, 4, input_t, weight_t>(params, stream);
    }
}
