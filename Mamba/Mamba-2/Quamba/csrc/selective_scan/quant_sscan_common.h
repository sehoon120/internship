...
inline __device__ char2 operator+(const char2 & a, const char2 & b){
    return {static_cast<char>(a.x + b.x), static_cast<char>(a.y + b.y)};
}
...
template<>
struct SSMScanOp<int8_t> {
    __device__ __forceinline__ char2 operator()(const char2 &ab0, const char2 &ab1) const {
        return make_char2(ab1.x * ab0.x, ab1.x * ab0.y + ab1.y);
    }
};
...
template <typename scalar_t> struct SSMScanPrefixCallbackOp {
    // using scan_t = std::conditional_t<std::is_same_v<scalar_t, float>, float2, float4>;  // not support fp16 by now
    using scan_t = std::conditional_t<std::is_same_v<scalar_t, int8_t>, char2, std::conditional_t<std::is_same_v<scalar_t, float>, float2, float4> >;
...
inline __device__ void load_weight(typename Ktraits::input_t *Bvar,
    ...
    #pragma unroll
    for (int i = 0; i < kNItems; ++i) { B_vals[i] = B_vals_load[i]; } // not sure why we have to convert it to float
...
template<typename Ktraits>
inline __device__ void store_output(at::Half *out,
                                    const at::Half (&out_vals)[Ktraits::kNItems],
                                    typename Ktraits::BlockStoreT::TempStorage &smem_store,
                                    int seqlen) {
    at::Half write_vals[Ktraits::kNItems];
    #pragma unroll
    for (int i = 0; i < Ktraits::kNItems; ++i) { write_vals[i] = out_vals[i]; }
    if constexpr (Ktraits::kIsEvenLen) {
        auto& smem_store_vec = reinterpret_cast<typename Ktraits::BlockStoreVecT::TempStorage&>(smem_store);
        using vec_t = typename Ktraits::out_vec_t;
        Ktraits::BlockStoreVecT(smem_store_vec).Store(
            reinterpret_cast<vec_t*>(out),
            reinterpret_cast<vec_t(&)[Ktraits::kNLoads]>(write_vals)
       );
...
// // To prevent modifying bwd, so instead of using template<typename Ktraits, array_t>, we did a overloading here
// template<typename Ktraits>
// inline __device__ void store_output(typename Ktraits::input_t *out,
//                                     const int8_t (&out_vals)[Ktraits::kNItems],
//                                     typename Ktraits::BlockStoreT::TempStorage &smem_store,
//                                     int seqlen) {
//     typename Ktraits::input_t write_vals[Ktraits::kNItems];
//     #pragma unroll
