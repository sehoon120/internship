/*
 * Adapted from
 * https://github.com/vllm-project/vllm/blob/main/csrc/quantization/marlin/qqq/marlin_qqq_gemm_kernel.cu
...
inline __device__ uint16_t float2_to_int82(float2 f) {
  // No 8-bit constraints in CUDA PTX, this is a workaround
  // https://docs.nvidia.com/cuda/inline-ptx-assembly/index.html#constraints
  // https://docs.nvidia.com/cuda/parallel-thread-execution/#restricted-use-of-sub-word-sizes
  uint16_t h0, h1;
  asm volatile("cvt.rni.s8.f32 %0, %1;\n" : "=h"(h0) : "f"(f.x));
  asm volatile("cvt.rni.s8.f32 %0, %1;\n" : "=h"(h1) : "f"(f.y));
  uint16_t res = ((h1 << 8) & 0xFF00) | (h0 & 0x00FF);
  return res;
}
...
__global__ void Marlin(
    ...
    int4* __restrict__ C,        // int32 global_reduce buffer of shape
                           ...
                           // int32 dtype
    int4* __restrict__ D,              // int8 output buffer of shape mxn
    const float* __restrict__ s_tok,   // fp32 activation per-token quantization
                                       // scales of shape mx1 or 1
    bool is_per_token_scale,           // whether s_tok is per-token or per-tensor
  ...
  if (slice_col_par >= n_tiles) {
    ...
    if (output_transpose) {
      D += ((slice_col_par / n_tiles) * 16 * thread_m_blocks) / 16;
    } else {
      D += (slice_col_par / n_tiles) * 16 * thread_m_blocks * prob_n / 16;
    }
  ...
  auto init_slice = [&]() {
    ...
    if (slice_col == n_tiles) {
      ...
      if (output_transpose) {
        D += (16 * thread_m_blocks) / 16;
      } else {
        D += 16 * thread_m_blocks * prob_n / 16;
      }
      if (is_per_token_scale)
  ...
  float a_per_tensor_scale = 0;
  if (!is_per_token_scale) { // not per-token scale -> per-tensor scale
    a_per_tensor_scale = *s_tok; // load per-tensor scale to register
  }
  ...
  auto write_result = [&]() {
    ...
    int d_gl_stride, d_gl_wr_delta, d_gl_wr, d_gl_wr_end;
    if (output_transpose) {
      d_gl_stride = tot_m;
      d_gl_wr_delta = (threads / (thread_n_blocks));
      d_gl_wr = (threadIdx.x / (thread_n_blocks)) +
                  16 * d_gl_stride * (threadIdx.x % (thread_n_blocks));
      d_gl_wr += 16 * d_gl_stride * (thread_n_blocks) * slice_col;
      d_gl_wr_end = d_gl_stride * prob_n;
    } else {
      d_gl_stride = prob_n / 16;
      d_gl_wr_delta = d_gl_stride * (threads / (thread_n_blocks));
      d_gl_wr = d_gl_stride * (threadIdx.x / (thread_n_blocks)) +
                    (threadIdx.x % (thread_n_blocks));
      d_gl_wr += (thread_n_blocks) * slice_col;
      d_gl_wr_end = d_gl_stride * prob_m;
    }
     // constexpr int d_sh_stride = 2 * thread_n_blocks + 1;
    // write result to shared memory
    constexpr int d_sh_wr_stride = 2 * thread_n_blocks;
    int d_sh_wr = (4 * d_sh_wr_stride) * ((threadIdx.x % 32) / 4) + (threadIdx.x % 32) % 4;
    d_sh_wr += 32 * (threadIdx.x / 32);
    // read result from shared memory
    constexpr int d_sh_rd_stride = thread_n_blocks;
    constexpr int d_sh_rd_delta = d_sh_rd_stride * (threads / thread_n_blocks);
    int d_sh_rd = d_sh_rd_stride * (threadIdx.x / (thread_n_blocks)) + (threadIdx.x % (thread_n_blocks));
    ...
    auto write = [&](int idx, int c0, int c1, float a_s, FragS_CHANNEL& w_s) {
      ...
      ((uint16_t*)sh)[idx] = float2_to_int82(deq_res);
    };
  ...
  #pragma unroll
        for (int j = 0; j < 4; j++) {
          int wr = d_sh_wr + 8 * j;
          write(wr + (4 * d_sh_wr_stride) * 0 + 0, frag_c[i][j][0][0],
                frag_c[i][j][0][1], frag_s_tok[i][0],
                frag_s_ch[j / 2][2 * (j % 2) + 0]);
          write(wr + (4 * d_sh_wr_stride) * 8 + 0, frag_c[i][j][0][2],
                frag_c[i][j][0][3], frag_s_tok[i][1],
                frag_s_ch[j / 2][2 * (j % 2) + 0]);
          write(wr + (4 * d_sh_wr_stride) * 0 + 4, frag_c[i][j][1][0],
                frag_c[i][j][1][1], frag_s_tok[i][0],
                frag_s_ch[j / 2][2 * (j % 2) + 1]);
          write(wr + (4 * d_sh_wr_stride) * 8 + 4, frag_c[i][j][1][2],
                frag_c[i][j][1][3], frag_s_tok[i][1],
                frag_s_ch[j / 2][2 * (j % 2) + 1]);
        }
        d_sh_wr += 16 * (4 * d_sh_wr_stride);
      }
    }
    __syncthreads();
  ...
  #pragma unroll
    for (int i = 0;
         i < ceildiv(16 * thread_m_blocks, threads / thread_n_blocks);
         i++) {
      if (output_transpose) {
        int d_gl_wr_end_m = 16 * d_gl_stride * (thread_n_blocks) * slice_col
                            + 16 * d_gl_stride * (threadIdx.x % (thread_n_blocks))
                            + prob_m;
        if (d_gl_wr < d_gl_wr_end_m) {
          for (int j = 0; j < 16; j++) {
            if (d_gl_wr + j*d_gl_stride < d_gl_wr_end) {
              *(reinterpret_cast<int8_t*>(D) + d_gl_wr + j*d_gl_stride)
                = *(reinterpret_cast<int8_t*>(sh+d_sh_rd) + j);
            }
          }
  ...
  #pragma unroll
    ...
    if (slice_iters == 0) {
      ...
      // write-out
      if (last) {
        if (s_tok_sh_wr_pred && is_per_token_scale) {
  ...
  #pragma unroll
          for (int i = 0; i < thread_m_blocks; i++) {
            if (is_per_token_scale) {
              ...
              frag_s_tok[i][0] = a_per_tensor_scale;
              frag_s_tok[i][1] = a_per_tensor_scale;
            }
...
#define __CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS,           \
    ...
    Marlin<NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS,     \
        ...
        <<<blocks, NUM_THREADS, max_shared_mem, stream>>>(                     \
            A_ptr, B_ptr, C_ptr, D_ptr, s_tok_ptr, is_per_token_scale,         \
            s_ch_ptr, s_group_ptr, prob_m, prob_n, prob_k,                     \
...
void marlin_qqq_cuda(const void* A, const void* B, void* C, void* D,
                     void* s_tok, bool is_per_token_scale, void* s_ch,
                     void* s_group, int prob_m, int prob_n, int prob_k,
                     bool output_transpose, void* workspace, int groupsize = -1,
  ...
  for (int i = 0; i < tot_m_blocks; i += 4) {
    ...
    if (output_transpose) {
      D_ptr += (16 * thread_m_blocks * par) / 16;
    } else {
      D_ptr += 16 * thread_m_blocks * (prob_n / 16) * par;
    }
    if (is_per_token_scale)
...
torch::Tensor w4a8o8_gemm(torch::Tensor const& a,
  ...
  TORCH_CHECK(size_m == a.size(0),
              "Shape mismatch: a.size(0) = " + str(a.size(0)) +
                  ", size_m = " + str(size_m));
  bool is_per_token_scale;
  if (s_tok.numel() != 1) {
    TORCH_CHECK(size_m == s_tok.numel(),
                "Shape mismatch: s_tok.numel() = " + str(s_tok.numel()) +
                    ", size_m = " + str(size_m));
    is_per_token_scale = true;
  } else {
    TORCH_CHECK(s_tok.numel() == 1, "s_tok.numel() = " + str(s_tok.numel()) +
                                        " is not 1 or equal to size_m = " +
                                        str(size_m));
    is_per_token_scale = false;
  }
  ...
  TORCH_CHECK(size_k == a.size(1),
              "Shape mismatch: a.size(1) = " + str(a.size(1)) +
                  ", size_k = " + str(size_k));
  TORCH_CHECK(size_k % W4A8O8::tile_size == 0,
              "size_k = " + str(size_k) +
                  " is not divisible by tile_size = " + str(W4A8O8::tile_size));
  TORCH_CHECK(
      (size_k / W4A8O8::tile_size) == b_q_weight.size(0),
      "Shape mismatch: b_q_weight.size(0) = " + str(b_q_weight.size(0)) +
          ", size_k = " + str(size_k) + ", tile_size = " + str(W4A8O8::tile_size));
  ...
  TORCH_CHECK(s_ch.numel() == size_n,
              "Shape mismatch: s_ch.numel() = " + str(s_ch.numel()) +
                  ", size_n = " + str(size_n));
  TORCH_CHECK(b_q_weight.size(1) % W4A8O8::tile_size == 0,
  ...
  int actual_size_n = (b_q_weight.size(1) / W4A8O8::tile_size) * W4A8O8::pack_factor_4bit;
  ...
  TORCH_CHECK(size_n % W4A8O8::min_thread_n == 0,
              "size_n = " + str(size_n) +
                  ", is not divisible by min_thread_n = " + str(W4A8O8::min_thread_n));
  int min_workspace_size = (size_n / W4A8O8::min_thread_n) * W4A8O8::max_par;
  ...
  torch::Tensor c = torch::empty({W4A8O8::max_par * 64, size_n}, options_c);

  // Alloc D matrix
  auto options_d = torch::TensorOptions().dtype(torch::kInt8).device(a.device());
  torch::Tensor d;
  if (output_transpose) {
  ...
  W4A8O8::marlin_qqq_cuda(
      a.data_ptr(), b_q_weight.data_ptr(), c.data_ptr(), d.data_ptr(),
      s_tok.data_ptr(), is_per_token_scale, s_ch.data_ptr(), s_group.data_ptr(), size_m, size_n,
      size_k, output_transpose, workspace.data_ptr(), groupsize, dev,
      at::cuda::getCurrentCUDAStream(dev), thread_k, thread_n, sms, W4A8O8::max_par);
