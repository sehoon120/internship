...
__global__ void Marlin(
  ...
  auto write_result = [&]() {
    ...
    if (output_transpose) {
      d_gl_stride = tot_m;
      d_gl_wr_delta = (threads / (2 * thread_n_blocks));
      d_gl_wr = (threadIdx.x / (2 * thread_n_blocks)) +
                  8 * d_gl_stride * (threadIdx.x % (2 * thread_n_blocks));
      d_gl_wr += 8 * d_gl_stride * (2 * thread_n_blocks) * slice_col;
  ...
  #pragma unroll
    for (int i = 0;
         ...
         i++) {
      if (output_transpose) {
        int d_gl_wr_end_m = 8 * d_gl_stride * (2 * thread_n_blocks) * slice_col
                            + 8 * d_gl_stride * (threadIdx.x % (2 * thread_n_blocks))
                            + prob_m;
        if (d_gl_wr < d_gl_wr_end_m) {
          for (int j = 0; j < 8; j++) {
            if (d_gl_wr + j*d_gl_stride < d_gl_wr_end) {
              *(reinterpret_cast<half*>(D) + d_gl_wr + j*d_gl_stride)
                = *(reinterpret_cast<half*>(sh+d_sh_rd) + j);
            }
          }
...
void marlin_qqq_cuda(const void* A, const void* B, void* C, void* D,
                     void* s_tok, bool is_per_token_scale,
                     void* s_ch, void* s_group, int prob_m,
                     int prob_n, int prob_k, bool output_transpose, void* workspace,
...
torch::Tensor w4a8o16_gemm(torch::Tensor const& a,
  ...
  TORCH_CHECK(size_k == a.size(1),
              "Shape mismatch: a.size(1) = " + str(a.size(1)) +
                  ", size_k = " + str(size_k));
  TORCH_CHECK(size_k % W4A8O16::tile_size == 0,
              "size_k = " + str(size_k) +
                  " is not divisible by tile_size = " + str(W4A8O16::tile_size));
  TORCH_CHECK(
      (size_k / W4A8O16::tile_size) == b_q_weight.size(0),
      "Shape mismatch: b_q_weight.size(0) = " + str(b_q_weight.size(0)) +
          ", size_k = " + str(size_k) + ", tile_size = " + str(W4A8O16::tile_size));
  ...
  TORCH_CHECK(s_ch.numel() == size_n,
              "Shape mismatch: s_ch.numel() = " + str(s_ch.numel()) +
                  ", size_n = " + str(size_n));
  TORCH_CHECK(b_q_weight.size(1) % W4A8O16::tile_size == 0,
  ...
  int actual_size_n = (b_q_weight.size(1) / W4A8O16::tile_size) * W4A8O16::pack_factor_4bit;
  ...
  TORCH_CHECK(size_n % W4A8O16::min_thread_n == 0,
              "size_n = " + str(size_n) +
                  ", is not divisible by min_thread_n = " + str(W4A8O16::min_thread_n));
  int min_workspace_size = (size_n / W4A8O16::min_thread_n) * W4A8O16::max_par;
  ...
  torch::Tensor c = torch::empty({W4A8O16::max_par * 64, size_n}, options_c);
  ...
  W4A8O16::marlin_qqq_cuda(
      ...
      at::cuda::getCurrentCUDAStream(dev), thread_k, thread_n, sms, W4A8O16::max_par);
