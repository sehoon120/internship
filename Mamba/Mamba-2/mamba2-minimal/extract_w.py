...
save_path = os.path.join(model_dir, "mamba2_2.7b_quantized_FXP.pth")
...
def quantize_tensor_asym(tensor, num_bits=8):
    ...
    scale = 1 / (2**6)  # 0.015625
    zero_point = qmin - min_val / (scale)
    zero_point = torch.clamp(zero_point.round(), qmin, qmax).int()

    # quantize
    q_tensor = torch.round(tensor / scale + zero_point).clamp(-128, 127).to(torch.int8)
...
for i in range(n_layers):
    ...
    with torch.no_grad():
        ...
        q, sc = fxp8_6.quantize(mixer.conv1d.weight), fxp8_6.scale
        ...
        q, sc = fxp16_14.quantize(norm.weight), fxp16_14.scale
        norm.weight.copy_(q)
